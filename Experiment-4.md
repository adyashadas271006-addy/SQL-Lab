...
## Query 1: Display the list of employees who have joined the company before 30th June 1980 or after 31st December 1981
```sql
...
```
---
## Query 2: Display the names of employees whose names have second alphabet 'A'
```sql
SELECT ename
FROM employee
WHERE ename LIKE '_A%';
...
```
---
## Query 4: Display the names of employees whose names have second alphabet 'A'
```sql
SELECT ename
FROM employee
WHERE ename LIKE '_A%';
...
```
---
## Query 7: Display name, salary, HRA, DA, PF, and total salary for each employee
```sql
SELECT 
    ename,
    sal,
